...
exports.post = async (req, res, next) => {
  try {
    ...
    const _item = await item.upsertItem(scanReq, method || "SCAN");
    ...
    method === "EDIT" && (response.data = await item.refreshItem(_item));
    res.status(response.code).send(response);
    if (_package.cahProduct) {
      await _package.populate("cahProduct");
      cahProduct.updateProduct(_package.cahProduct);
    }
    if (_package.psPackage) {
      await _package.populate("psPackage");
      psPackage.updatePsPackage(_package.psPackage);
    }
  } catch (e) {
    console.error(e);
    return res
